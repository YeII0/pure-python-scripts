...
async def dlSingleFile(formElem, cookies, dlPath, headers, rename):
            ...
            async with aiohttp.request(
                ...
                if rename == True:
                    filenameRe = re.compile(
                        r"(.+?)(_ep\d+)(_\d+)?(_.+?)(_AnimeSubInfo_id\d+.zip)$"
                    )
                ...
                subsZip.write(await r.read())
                subsZip.close()
                break
        except Exception as err:
            print(
                "Problem with downloading single file. Retry after 5 seconds."
            )
